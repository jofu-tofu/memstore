...
function formatInsightsSummary(summary: Awaited<ReturnType<typeof getInsightsSummary>>['value']): string {
  ...
  lines.push('=== MemStore Memory System Insights ===');
...
async function exportReport(
  ...
  const memstoreDir = process.env.MEMSTORE_DIR || join(homedir(), '.memstore');
  const reportsDir = join(memstoreDir, 'mem-store', 'reports');
...
function printHelp() {
  console.log(`
MemStore Memory System Insights
