 ...
 *
 * CRITICAL: ALWAYS exits 0 (never blocks Claude Code)
 */
...
import { logMemoryError } from '../lib/logging/error-logger';
...
/**
 * Gets MemStore directory path from environment or uses default.
 */
...
function spawnProcessor() {
  // Use import.meta.dir for path relative to this file
  const processorPath = join(import.meta.dir, 'process-queue.ts');
...
async function main() {
  ...
  try {
    ...
    const queueDir = join(getMemstoreDir(), 'mem-store', 'queue');
  ...
  // CRITICAL: Always exit 0 - never block Claude Code
  process.exit(0);
}
