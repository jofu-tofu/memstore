...
import { logCaptureOperation, type CaptureOperationMetadata } from '../lib/logging/operations-logger';
...
async function checkRetentionPolicy(config: MemoryConfig): Promise<void> {
  try {
    ...
    const checker = new RetentionPolicyChecker(memstoreDir);
...
async function main() {
  ...
  const lockFile = join(getMemstoreDir(), 'mem-store', 'queue', '.processor.lock');
