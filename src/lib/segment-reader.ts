...
/**
 * Get MemStore directory from environment or default
 */
...
export function getSegmentPath(segmentId: string): Result<string, QueryError> {
  try {
    ...
    // Construct path
    const segmentPath = join(getMemstoreDir(), 'mem-store', 'segments', yearMonth, `${segmentId}.md`);
...
export function readSessionSegments(
  ...
  try {
    ...
    const segmentsBasePath = join(getMemstoreDir(), 'mem-store', 'segments');
