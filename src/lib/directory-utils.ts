...
/**
 * Get MemStore directory from env or fallback to ~/.memstore
 */
 ...
 *
 * @param memstoreDir - Optional MemStore directory (uses env/default if not provided)
...
export function ensureMemStoreDirectories(
  ...
  try {
    const baseDir = memstoreDir || getMemstoreDir();
