...
function getSettingsPath(): string {
  return join(getMemstoreDir(), '.claude', 'settings.json');
}
...
async function persistConfigUpdate(
  ...
  try {
    ...
    const claudeDir = join(getMemstoreDir(), '.claude');
...
> {
  try {
    ...
    const experimentsDir = join(getMemstoreDir(), 'mem-store/metrics/experiments');
