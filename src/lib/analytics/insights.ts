...
} from '../logging/operations-logger';
...
async function loadAllSegmentMetadata(): Promise<SegmentInsight[]> {
  ...
  const segmentsPath = join(memstoreDir, 'mem-store', 'segments');
...
> {
  const memstoreDir = getMemstoreDir();
  const operationsPath = join(memstoreDir, 'mem-store', 'metrics', 'operations.jsonl');
