...
describe('Query Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    testMemstoreDir = join(homedir(), '.memstore-test-query-integration');
    mkdirSync(testMemstoreDir, { recursive: true });
    process.env.MEMSTORE_DIR = testMemstoreDir;

    // Create comprehensive test data
    const memStoreDir = join(testMemstoreDir, 'mem-store');
...
JWT tokens, session management, and RBAC setup.`;
  ...
  describe('Error Handling in Workflows', () => {
    ...
    test('should handle missing registry during workflow', () => {
      ...
      const registryPath = join(testMemstoreDir, 'mem-store', 'structured', 'session-registry.json');
    ...
    test('should handle missing keyword index during workflow', async () => {
      ...
      const indexPath = join(testMemstoreDir, 'mem-store', 'indexes', 'keyword', 'index.json');
