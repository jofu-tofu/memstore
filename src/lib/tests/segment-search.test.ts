...
describe('SegmentSearch', () => {
  ...
  beforeEach(() => {
    ...
    testMemstoreDir = join(homedir(), '.memstore-test-segment-search');
    mkdirSync(testMemstoreDir, { recursive: true });
    process.env.MEMSTORE_DIR = testMemstoreDir;

    // Create test keyword index
    const indexDir = join(testMemstoreDir, 'mem-store', 'indexes', 'keyword');
...
Never accessed segment content.`;
    ...
    test('should handle missing segments directory gracefully', async () => {
      ...
      const segmentsDir = join(testMemstoreDir, 'mem-store', 'segments');
