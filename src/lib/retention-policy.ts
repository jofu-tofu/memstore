...
export class RetentionPolicyChecker {
  private memstoreDir: string;
  private registryPath: string;

  constructor(memstoreDir?: string) {
    this.memstoreDir = memstoreDir || process.env.MEMSTORE_DIR || join(homedir(), '.memstore');
    this.registryPath = join(this.memstoreDir, 'mem-store', 'structured', 'session-registry.json');
  }
  ...
  private async saveRegistry(sessions: Record<string, SessionEntry>): Promise<void> {
    try {
      ...
      const registryDir = join(this.memstoreDir, 'mem-store', 'structured');
