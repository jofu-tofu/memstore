...
const TEST_MEMSTORE_DIR = join(homedir(), '.memstore-test-retrieval-logger');
...
describe('Retrieval Logger', () => {
  ...
  describe('logRetrieval()', () => {
    ...
    test('should handle logging errors gracefully', () => {
      // Invalid MEMSTORE_DIR path to trigger error
      const originalPaiDir = process.env.MEMSTORE_DIR;
      process.env.MEMSTORE_DIR = '/invalid/path/that/cannot/be/written';
      ...
      process.env.MEMSTORE_DIR = originalPaiDir;
    });
