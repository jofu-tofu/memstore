...
const TEST_MEMSTORE_DIR = join(homedir(), '.memstore-test-config');
const TEST_SETTINGS_PATH = join(TEST_MEMSTORE_DIR, '.claude', 'settings.json');

describe('config', () => {
  beforeAll(() => {
    // Create test MemStore directory structure
  ...
  describe('getMemoryConfig', () => {
    test('should return default config when settings.json missing', async () => {
      ...
      // Act: Import with test MEMSTORE_DIR
    ...
    test('should load valid configuration from settings.json', async () => {
      ...
      // Act: Load config with test MEMSTORE_DIR
      const originalMemstoreDir = process.env.MEMSTORE_DIR;
      process.env.MEMSTORE_DIR = TEST_MEMSTORE_DIR;
    ...
    test('should use MEMSTORE_DIR environment variable if set', async () => {
      // Arrange: Set custom MEMSTORE_DIR and create config there
      const customMemstoreDir = join(homedir(), '.memstore-test-custom');
      const customSettingsPath = join(customMemstoreDir, '.claude', 'settings.json');

      mkdirSync(join(customMemstoreDir, '.claude'), { recursive: true });
      ...
      // Act: Load config with custom MEMSTORE_DIR
      const originalMemstoreDir = process.env.MEMSTORE_DIR;
      process.env.MEMSTORE_DIR = customMemstoreDir;
