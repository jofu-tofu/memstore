...
import type { Result, ProviderError } from '../types/common';
import { globalProviderRegistry } from './provider-registry';
import { getMemoryConfig, MemoryConfig } from './config';
import { logCaptureOperation, type CaptureOperationMetadata, type ProviderTiming } from '../lib/logging/operations-logger';
...
async function updateSessionRegistry(
  ...
  const registryPath = join(memstoreDir, 'mem-store', 'structured', 'session-registry.json');
  ...
  try {
    ...
    const registryDir = join(memstoreDir, 'mem-store', 'structured');
