...
const TEST_MEMSTORE_DIR = join(homedir(), '.memstore-test-keyword-index');
...
describe('providers/storage/keyword-index.ts', () => {
  ...
  beforeEach(async () => {
    ...
    indexManager = new KeywordIndexManager(TEST_MEMSTORE_DIR);
  ...
  test('should persist index to disk', async () => {
    ...
    const indexManager2 = new KeywordIndexManager(TEST_MEMSTORE_DIR);
  ...
  test('should return empty index when file does not exist', async () => {
    const newManager = new KeywordIndexManager(TEST_MEMSTORE_DIR);
  ...
  test('should handle MEMSTORE_DIR environment variable fallback', () => {
    // Don't pass memstoreDir to constructor
    const envManager = new KeywordIndexManager();

    // Should use process.env.MEMSTORE_DIR or fallback to ~/.memstore
