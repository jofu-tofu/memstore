/**
 * SQLite Storage Backend Example for the MemStore Memory System
 *
 ...
 *      dbPath: join(getMemstoreDir(), 'mem-store', 'segments.db')
 *    }
 *    ```
 *
...
export interface SQLiteBackendConfig {
   ...
   * Default: {memstoreDir}/mem-store/segments.db
   */
  dbPath?: string;
...
export class SQLiteBackendProvider implements StorageProvider {
  ...
  constructor(config: SQLiteBackendConfig = {}) {
    const memstoreDir = config.memstoreDir || getMemstoreDir();
    this.config = {
      dbPath: config.dbPath || join(memstoreDir, 'mem-store', 'segments.db'),
      memstoreDir,
