/**
 * Keyword Search Provider for the MemStore Memory System
 *
...
export class KeywordSearch implements SearchProvider {
  ...
  constructor(options?: { memstoreDir?: string; customStopWords?: Set<string> }) {
    const memstoreDir = options?.memstoreDir || getMemstoreDir();
    this.indexPath = join(memstoreDir, 'mem-store', 'indexes', 'keyword', 'index.json');
