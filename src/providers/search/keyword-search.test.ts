...
const TEST_MEMSTORE_DIR = join(homedir(), '.memstore-test-keyword-search');
const TEST_INDEX_DIR = join(TEST_MEMSTORE_DIR, 'mem-store', 'indexes', 'keyword');
...
describe('KeywordSearch Provider', () => {
  ...
  beforeEach(async () => {
    ...
    provider = new KeywordSearch({ memstoreDir: TEST_MEMSTORE_DIR });
  ...
  describe('initialization', () => {
    ...
    test('should handle missing index file', async () => {
      ...
      const emptyProvider = new KeywordSearch({
        memstoreDir: join(TEST_MEMSTORE_DIR, 'nonexistent')
      });
    ...
    test('should handle corrupted index file gracefully (Story 3.6)', async () => {
      const corruptDir = join(TEST_MEMSTORE_DIR, 'corrupt');
      ...
      const corruptProvider = new KeywordSearch({ memstoreDir: corruptDir });
  ...
  describe('search', () => {
    ...
    test('should complete search in < 100ms for large index (10K segments)', async () => {
      ...
      const largeIndexDir = join(TEST_MEMSTORE_DIR, 'large-index', 'mem-store', 'indexes', 'keyword');
      ...
      const largeProvider = new KeywordSearch({ memstoreDir: join(TEST_MEMSTORE_DIR, 'large-index') });
      ...
      // Cleanup
      rmSync(join(TEST_MEMSTORE_DIR, 'large-index'), { recursive: true, force: true });

      // Validate < 100ms requirement (AC #5)
      expect(elapsed).toBeLessThan(100);
    });

    test('should handle MEMSTORE_DIR environment variable', async () => {
      // This test validates MEMSTORE_DIR support exists
      const customProvider = new KeywordSearch({ memstoreDir: TEST_MEMSTORE_DIR });
  ...
  describe('Debug logging (Story 4.6)', () => {
    beforeAll(() => {
      ...
      const configDir = join(TEST_MEMSTORE_DIR, '.claude');
      ...
      // Set MEMSTORE_DIR to use test config
